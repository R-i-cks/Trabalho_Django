...
def run():
    ...
    apelidos = ["Silva", "Santos", "Oliveira", "Ferreira", "Pereira", "Costa", "Martins", "Rodrigues",
                            "Almeida", "Ribeiro"]
    especialidades = ["Cardiologia", "Clinica Geral",
                               "Pediatria",  "Cirurgia Geral"]

    dosagens = ["500mg", "200mg", "250mg", "20mg", "1000mg", "10mg", "325mg"]
    fabricantes = ["Johnson & Johnson", "Pfizer", "GlaxoSmithKline",
    ...
    for i in range(1,51):  # criar utente e respetivo utilizador
        ...
        u = Utente(nome=f"utente_{i}",email=f"utente_{i}@email.com",genero=genero,
                   primeiro_nome=primeiro,apelido=random.choice(apelidos),
                   data_nascimento=data_aleatoria(ano_limite=2014), telemovel=telemovel(),
                   contacto_emergencia =telemovel(), tem_seguro = random.choice([True,False]))
        user = User.objects.create_user(f"utente_{i}",f"utente_{i}@email.com", "password")
    ...
    for i in range(1,51):
        ...
        m = Medico(nome=f"medico_{i}", email=f"medico_{i}@email.com",genero=genero,
                   primeiro_nome=primeiro,apelido=random.choice(apelidos),
                   data_nascimento=data_aleatoria(ano_limite=2014), telemovel=telemovel(),
                   especialidade=random.choice(especialidades))
        user = User.objects.create_user(f"medico_{i}", f"medico_{i}@email.com", "password")
    ...
    for i in range(1,51):
        ...
        e = Enfermeiro(nome=f"enfermeiro_{i}", email=f"enfermeiro_{i}@email.com",genero=genero,
                   primeiro_nome=primeiro,apelido=random.choice(apelidos),
                   data_nascimento=data_aleatoria(ano_limite=2014), telemovel=telemovel())
        user = User.objects.create_user(f"enfermeiro_{i}",f"enfermeiro_{i}@email.com", "password")
    ...
    for i in range(1, 51): # Auxiliar
        ...
        a = Auxiliar(nome=f"auxiliar_{i}", email=f"auxiliar_{i}@email.com", genero=genero,
                       primeiro_nome=primeiro, apelido=random.choice(apelidos),
                       data_nascimento=data_aleatoria(ano_limite=2014), telemovel=telemovel())
        user = User.objects.create_user(f"auxiliar_{i}", f"auxiliar_{i}@email.com", "password")
    ...
    for i in range(1, 51):   # Familiar
        ...
        f = Familiar(nome=f"familiar_{i}", email=f"familiar_{i}@email.com", genero=genero,
                       primeiro_nome=primeiro, apelido=random.choice(apelidos),
                       data_nascimento=data_aleatoria(ano_limite=2014), telemovel=telemovel())
        f.save()
        utente = Utente.objects.all().filter(nome=f"utente_{i}").first()
        f.utente.add(utente)
        user = User.objects.create_user(f"familiar_{i}", f"familiar_{i}@email.com", "password")
    ...
    for i in range(1,51):
        med = Medicamento(nome=f"medicamento {i}", dosagem=random.choice(dosagens),
    ...
    for i in range(1,51):
        ...
        for j in range(2):
            ...
            if random.randrange(0, 50) > 25:
                ...
                enfermeiro = Enfermeiro.objects.all().filter(nome=f"enfermeiro_{i}").first()
        ...
        for a in range(2):
            ...
            medicamento = Medicamento.objects.all().filter(nome=f"medicamento {random.randint(0,50)}").first()
            prescricao = Prescricoes(utente=u,medico=m, inicio_toma=data_aleatoria(2023, 2024),
        ...
        exames = ["Hemograma", "Colesterol Total", "Glicemia em Jejum",
                  "Creatinina", "Hemoglobina Glicada"]

        exames_resultados = {
            "Hemograma": ["Normal", "Anemia leve", "Leucocitose", "Trombocitopenia", "Normal"],
            "Colesterol Total": ["Desejável", "Limítrofe", "Alto", "Muito Alto", "Desejável"],
            "Glicemia em Jejum": ["Normal", "Pré-diabetes", "Diabetes", "Normal", "Pré-diabetes"],
            "Creatinina": ["Normal", "Levemente elevada", "Elevada", "Normal", "Levemente elevada"],
            "Hemoglobina Glicada": ["Normal", "Pré-diabetes", "Diabetes", "Normal", "Pré-diabetes"]
        }

        for b in range(2):
            exame = random.choice(exames)
            resultado = random.choice(exames_resultados[exame])
            e = Exame(utente=u, data_realizacao=data_aleatoria(2023, 2024),
                       nome_exame=exame, resultado=resultado)
